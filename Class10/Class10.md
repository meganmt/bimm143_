...
There are some things in this data set that we will ignore, including
the ID and Diagnosis columns and weird last X column. We only want
columns 3-32
...
Before we do PCA we need to think/consider whether we should SCALE our
input
...
The first PC captures 44.27% of the proportion of the original variance.
72.67% in the first 3 PCs.
...
Let’s try to combine the results of PCA with clustering to make it look
better. Let’s see if PCA improves or degrades the performance of
...
![](Class10_files/figure-gfm/unnamed-chunk-16-1.png)<!-- --> Looks like:
black and red correspond to malignant green corresponds to benign
...
We will use predict() to take our PCA model and new cancer cell data and
project that data onto our PCA space
...
Use the predict function with previous PCA model and new data and draw
the PCA plot again and add new data
